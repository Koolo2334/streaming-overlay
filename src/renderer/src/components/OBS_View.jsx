...
const OBS_View = () => {
  ...
  useEffect(() => {
    ...
    window.api.onAvatarImageUpdate(url => {
        setAvatarUrl(url)
    })
  ...
  const BOOT_PHASES = [
    {
      ...
      baseProgress: 0,
      steps: [
        { text: "BIOS DATE 01/01/2025 12:34:56 VER 2.1.0", delay: 800 },
        { text: "CPU: QUANTUM CORE i99 128-CORE DETECTED", delay: 600 },
        { text: "CHECKING NVRAM..", delay: 400, suffix: " [ OK ]" },
        { text: "DRAM FREQUENCY: 6400MHz", delay: 400 },
        { text: "USB CONTROLLER INITIALIZED", delay: 300 }
      ]
    },
    {
      name: "DRIVERS_LOAD",
      baseProgress: 20,
      steps: [
        { text: "LOADING KERNEL IMAGE", delay: 1200, progressAdd: 5, suffix: "... 100%" },
        { text: "MOUNTING FILESYSTEM [ROOT]", delay: 800, suffix: " [ OK ]" },
        { text: "INIT: GPU DRIVER (RTX_9090_TI)", delay: 1500, progressAdd: 10 },
        { text: "INIT: AUDIO SUBSYSTEM", delay: 600 },
        { text: "INIT: NETWORK ADAPTER (10GbE)", delay: 900 }
      ]
    },
    {
      ...
      baseProgress: 60,
      steps: [
        { text: "STARTING: POWER MANAGEMENT", delay: 400, suffix: " [ OK ]" },
        { text: "STARTING: CRON DAEMON", delay: 300, suffix: " [ OK ]" },
        { text: "STARTING: SECURITY AUDIT", delay: 600 },
        { text: "CONNECTING TO STREAM SERVERS...", delay: 2000, progressAdd: 15, suffix: " CONNECTED" },
        { text: "SYNCING TIME WITH NTP...", delay: 500, suffix: " [ OK ]" }
      ]
    },
    {
      name: "USER_SPACE",
      baseProgress: 90,
      steps: [
        { text: "PREPARING USER SESSION", delay: 800 },
        { text: "LOADING OVERLAY CONFIGURATION", delay: 600 },
        { text: "WAITING FOR SYSTEM READY...", delay: 1500 }
      ]
    }
  ]
  ...
  const runBootSequence = async () => {
    ...
    if (step.suffix) {
        displayLine += step.suffix
    } else {
        displayLine += ` ... ${Math.floor(nextP)}%`
    }
    ...
    if (nextTextIndex >= currentPhaseData.steps.length) {
      bootStateRef.current.phase = phase + 1
      bootStateRef.current.textIndex = 0
    } else {
      bootStateRef.current.textIndex = nextTextIndex
    }
  ...
  const wait = (ms) => new Promise(resolve => {
    bootStateRef.current.timer = setTimeout(resolve, ms)
  })
  ...
  return (
    // ★修正: 外側コンテナをFlexboxで中央寄せ
    <div className={`obs-container`}>
      
      {/* ★修正: 1920x1080のビューポートを定義 */}
      <div className="obs-viewport">
        ...
        <div className="bg-mask-container">
          ...
          {!gameActive && (
            <div className="bg-part center" style={{ top: y, left: x, width: width, height: height, zIndex: 1 }} />
          )}
        </div>
        ...
        <div className={`scene-content main-scene ${visualMode === 'main' ? 'active' : ''}`}>
          ...
          <div className="avatar-area" style={{ left: avatarX, top: avatarY, width: avatarW, height: avatarH }}>
            <div className="avatar-placeholder" style={{ position: 'relative', width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <canvas ref={audioCanvasRef} className="voice-canvas" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', pointerEvents: 'none', zIndex: 5 }} />
              <div 
      ...
      <style>{`
        ...
        .game-window-body.transparent-body { 
        ...
        /* --- MAIN SCENE STARTUP ANIMATIONS --- */
        .main-scene .tech-window, .main-scene .avatar-area, .main-scene .target-zone-container {
          opacity: 0; 
          transform: scale(0.9);
        }
        ...
        .avatar-circle { width: 100px; height: 100px; background: #44475a; border: 3px solid var(--col-text); border-radius: 50%; background-size: cover; background-position: center; z-index: 10; position: relative; }
